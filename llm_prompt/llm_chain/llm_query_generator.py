...
class JsonToDict(BaseOutputParser):
    ...
    def parse(self, text: str):
        ...
        except json.JSONDecodeError:  # Specify the exception for clarity
            output = text
        return output
    
# Define the query dictionary to match the query types and their definitions
query_dict = {
...
def LLM_Query_generator(query_type: str) -> Dict[str, Union[str, list]]:
    """Generate queries based on the provided type and return as a dictionary.

    Args:
        query_type (str): The type of the query. Expected values are 'general', 'specific', or 'detail'.

    Returns:
        Dict[str, Union[str, list]]: A dictionary where keys represent genres and values represent 
                                     natural language descriptions.

    Raises:
        ValueError: If the provided query_type is not 'general', 'specific', or 'detail'.
    """

    # Check if the provided query_type is valid
    if query_type not in query_dict:
        raise ValueError("Invalid query type. Expected 'general', 'specific', or 'detail'.")
    
    query = query_dict[query_type]
    chat_template = ChatPromptTemplate.from_messages([
        SystemMessagePromptTemplate.from_template(system_template)])

    LLM_query_chain = LLMChain(
        llm=ChatOpenAI(model_name="gpt-3.5-turbo", temperature=0),
        prompt=chat_template,
        output_parser=JsonToDict()
    )
    
    return LLM_query_chain.run({'format': query})
